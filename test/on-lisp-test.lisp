...
(define-test test-with-inference%%-expand
  (assert-expands
   (WITH-GENSYMS (?X)
     ...
     (=BIND (#:G1)
         (=BIND (#:G2)
             (ON-LISP.24::PROVE% (LIST 'BIG ?X) NIL)
           (=BIND (#:G3)
               (ON-LISP.24::PROVE% (LIST 'RED ?X) #:G2)
             (=VALUES #:G3)))
...
(deftest test-with-inference ()
  ...
  (<- (factorial ?n ?f)
      (lisp (> ?n 0))
      (on-lisp.24::is ?n1 (- ?n 1))
      (factorial ?n1 ?f1)
      (on-lisp.24::is ?f (* ?n ?f1)))
  ;; p. 344
  (is (equal
       (let ((results nil))
